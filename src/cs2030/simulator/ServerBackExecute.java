        ...
        if (opServer.isPresent()) {
            ...
            if (customer == null) {
                if (server.getnumOfWaitingCustomer() == 0) {
                    Server newserver = new Server(server.getID(), true, false, server.getnumOfWaitingCustomer(), server.getNextAvailableTime(), server.getCustomerQueue());
            ...
            Server newserver = new Server(server.getID(), true, server.getnumOfWaitingCustomer(), server.getNextAvailableTime()+randomGen.genServiceTime(), server.getCustomerQueue());
            Event newEvent = new ServeEvent2(customer, server.getNextAvailableTime(), server.getID());
            Shop newShop = shop.replace(newserver);
